...
def cadastrar():
    ...
    if (len(dados_lidos) > 0):
        ...
        if (v_senha == c_senha):
            ...
            cursor = banco.cursor()
            comando_SQL = "INSERT INTO cad_oficina (cnpj_cpf,i_estadual,nome_oficina,senha,responsavel,endereco,bairro,cidade,estado,cep,fone_oficina,whatsapp_oficina,email_oficina,ativo_s_n_susp) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            dados = (str(v_login),str(v_inscricao),str(v_nome_oficina),str(v_senha),str(v_responsavel),str(v_endereco),str(v_bairro),str(v_cidade),str(v_estado),str(v_cep),str(v_fone_oficina),str(v_whatsapp_oficina),str(v_email_oficina),str(v_ativo))
